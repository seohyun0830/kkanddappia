...
def f_guide(window, flag, isTab, invenList, isDragging):
    ...
    if (texts[f_guide.idx] != ""):
        ...
        if (f_guide.idx == 0 or f_guide.idx == 1):
            ...
            for i in range(len(invenList)):
                if invenList[i] == 5:
                    x = 120 * (i + 1)
                    break
            if (isTab):
                pygame.draw.rect(box, hole_color, (x, 90, 120,120))
            elif (isDragging):
                pygame.draw.rect(box, hole_color, (0, 0, 1200, 800))
        elif (f_guide.idx == 4):
